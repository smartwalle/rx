...
//
// Copyright (c) 2014 Manuel Martínez-Almeida. All rights reserved.
...
walk:
	for {
		...
		//if n.route != nil {
		//	panic("route are already registered for path '" + fullPath + "'")
		//}
		n.route = route
		return
	}
...
func (n *node) insertChild(numParams uint8, path string, fullPath string, route *Route) {
	for numParams > 0 {
...
func (n *node) findCaseInsensitivePath(path string, fixTrailingSlash bool) (ciPath []byte, found bool) {
	...
	for len(path) >= len(n.path) && strings.EqualFold(path[:len(n.path)], n.path) {
		...
		if !n.wildChild {
			...
			found = fixTrailingSlash && path == "/" && n.route != nil
			return
		}
